#!/usr/bin/env python3
"""
Scraper rápido de cuotas Mundial 2026 — OddsChecker
Obtiene 1X2 (Win Market) + Marcador Exacto (Correct Score)
con 27+ casas de apuesta por partido, en paralelo.
"""
...
def best_per_bk(market_data, field='line'):
    """Para cada bookmaker, devuelve la apuesta con menor cuota (más probable)"""
...
def consensus(by_bk):
    """Retorna el valor con más votos y nivel de consenso"""
    if not by_bk:
...
def process_match(match):
    ...
    try:
        ...
        # 1X2 — campo betName (Mexico / Draw / South Africa)
        win_by_bk = best_per_bk(win_data, field='betName') if win_data else {}
        # Correct Score — campo line (1-0, 2-1, etc.)
...
def main():
    print(f"Scrapeando {len(MATCHES)} partidos con 4 workers en paralelo...\n")
    t0 = time.time()
    results = [None] * len(MATCHES)

    with ThreadPoolExecutor(max_workers=4) as ex:
        futures = {ex.submit(process_match, m): i for i, m in enumerate(MATCHES)}
    ...
    lines = [
        ...
        "="*90,
        f"{'#':<3} {'Fecha':<8} {'Grp':<4} {'J':<2} {'Partido':<35} {'1X2':<12} {'CS':<10} {'Cons 1X2':<10} {'Cons CS'}",
        "-"*90,
    ]
    for i, r in enumerate(results, 1):
        if r:
            ...
            lines.append(
                ...
                f"{partido:<35} {str(r.get('win_pred','?')):<12} {str(r.get('cs_pred','?')):<10} "
                f"{r.get('win_nivel','?'):<10} {r.get('cs_nivel','?')}"
            )
